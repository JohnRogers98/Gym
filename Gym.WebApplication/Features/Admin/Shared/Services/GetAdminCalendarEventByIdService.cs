...
namespace Gym.WebApplication.Features.Admin.Shared.Services
{
    ...
    {
        Task<AdminCalendarItemViewModel?> ExecuteAsync(CalendarEventId calendarEventId, CancellationToken cancellationToken = default);
    }
    ...
    {
        public async Task<AdminCalendarItemViewModel?> ExecuteAsync(CalendarEventId calendarEventId, CancellationToken cancellationToken = default)
        {
            return await _pipelineProvider.CalendarEventEventualConsistency.ExecuteAsync(async innerToken =>
            {
                return await _decoratee.ExecuteAsync(calendarEventId, innerToken);
            }, cancellationToken);
        }
    }
    ...
    public class GetAdminCalendarEventByIdService(HttpClient _httpClient, IMapper _mapper) : IGetAdminCalendarEventByIdService
    {
        public async Task<AdminCalendarItemViewModel?> ExecuteAsync(CalendarEventId calendarEventId, CancellationToken cancellationToken = default)
        {
            var response = await _httpClient.GetFromJsonAsync<GetAdminCalendarEventResponse>($"api/admin-calendar-events/{calendarEventId.Value}", cancellationToken: cancellationToken);

            if (response is not null)
                return _mapper.Map<AdminCalendarItemViewModel>(response);
            return null;
        }
    }
}
