...
namespace Gym.WebApplication.Features.Calendar.Services
{
    ...
    {
        Task<Boolean> ExecuteAsync(CalendarItemViewModel calendarItem, CancellationToken cancellationToken = default);
    }

    public class BookCalendarItemService(HttpClient _httpClient) : IBookCalendarItemService
    {
        public async Task<Boolean> ExecuteAsync(CalendarItemViewModel calendarItem, CancellationToken cancellationToken = default)
        {
