...
namespace Gym.WebApplication.Features.Calendar.Services
{
    ...
    {
        Task<IEnumerable<CalendarItemViewModel>> ExecuteAsync(CancellationToken cancellationToken = default);
    }

    public class GetAllCalendarItemsService(HttpClient _httpClient, IMapper _mapper) : IGetAllCalendarItemsService
    {
        public async Task<IEnumerable<CalendarItemViewModel>> ExecuteAsync(CancellationToken cancellationToken = default)
        {
