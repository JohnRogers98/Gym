...
{
    public class ListSessionInstructorPersonalTrainingsService(IHttpClientFactory _httpClientFactory, IOptions<BffOptions> _bffOptions, IMapper _mapper) 
        : IRequestHandler<ListSessionInstructorPersonalTrainings, IEnumerable<PersonalTrainingViewModel>>
    {
        public async Task<AsyncOperation<IEnumerable<PersonalTrainingViewModel>>> HandleAsync(ListSessionInstructorPersonalTrainings request, CancellationToken cancellationToken = default)
        {
            using var httpClient = _httpClientFactory.CreateClient(_bffOptions.Value.ClientName);

            using var listPersonalTrainingsRequest = this.CreateGetRequest(_bffOptions.Value.ListSessionInstructorPersonalTrainigsEndpoint);
    ...
    public class ListSessionInstructorPersonalTrainings;
}
